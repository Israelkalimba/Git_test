...
$tranches_minerval_payees = [];
foreach ($frais_a_payer as $frais_item) {
    if (
        stripos($frais_item['type_frais'], 'Minerval') !== false &&
        ($frais_item['statut_paiement'] ?? '') === 'succes' &&
        preg_match('/Tranche\s*([0-9]+)/i', $frais_item['type_frais'], $match_tranche)
    ) {
        $tranches_minerval_payees[(int)$match_tranche[1]] = true;
    }
}
$tranche1_minerval_payee = !empty($tranches_minerval_payees[1]);
...
<body>
    <div class="etudiant-layout">
        ...
        <div class="main-content">
            <?php $navbar_notif_non_lues = $notifications_non_lues;
            include 'includes/navbar_etudiant.php'; ?>
            <main class="dashboard-content">
                <div class="page-header">
                    <div class="row align-items-center">
                        <div class="col-lg-7">
                            <h1 class="page-title"><i class="fas fa-credit-card"></i> Payer mes Frais Académiques</h1>
                            <p class="page-subtitle">Sélectionnez un frais à payer.</p>
                        </div>
                ...
                <div class="resume-paiement">
                    <div class="resume-item-paiement"><i class="fas fa-user-graduate"></i>
                        <div><span class="resume-label">Étudiant</span><strong><?= htmlspecialchars($etudiant_nom) ?></strong></div>
                    </div>
                    <div class="resume-item-paiement"><i class="fas fa-id-card"></i>
                        <div><span class="resume-label">Matricule</span><strong><?= htmlspecialchars($matricule) ?></strong></div>
                    </div>
                    <div class="resume-item-paiement"><i class="fas fa-layer-group"></i>
                        <div><span class="resume-label">Filière</span><strong><?= htmlspecialchars($etudiant['nom_filiere']) ?></strong></div>
                    </div>
                    <div class="resume-item-paiement"><i class="fas fa-graduation-cap"></i>
                        <div><span class="resume-label">Promotion</span><strong><?= htmlspecialchars($etudiant['nom_promotion']) ?></strong></div>
                    </div>
                </div>
                ...
                <div class="paiement-grid">
                    ...
                    <div class="frais-column">
                        ...
                        <?php if (empty($frais_restants)): ?>
                            <div class="all-paid-card"><i class="fas fa-check-circle"></i>
                                <h4>Félicitations !</h4>
                                <p>Tous vos frais ont été payés.</p>
                            </div>
                        <?php else: ?>
                            <div class="frais-list">
                                <?php foreach ($frais_restants as $frais):
                                    ...
                                    $numero_tranche = 0;
                                    preg_match('/Tranche\s*([0-9]+)/i', $frais['type_frais'], $match_tranche);
                                    if (!empty($match_tranche[1])) {
                                        $numero_tranche = (int)$match_tranche[1];
                                    }
                                    $est_minerval = stripos($frais['type_frais'], 'Minerval') !== false;
                                    $bloque_t1 = !$tranche1_minerval_payee && !$est_minerval;
                                    $bloque_tranche_precedente = $est_minerval && $numero_tranche > 1 && empty($tranches_minerval_payees[$numero_tranche - 1]);
                                    $tranche_bloquee = $bloque_t1 || $bloque_tranche_precedente;

                                    $texte_blocage = '';
                                    $message_blocage = '';
                                    if ($tranche_bloquee) {
                                        if ($bloque_t1) {
                                            $texte_blocage = 'Bloqué jusqu\'au payement de la tranche 1';
                                            $message_blocage = 'Vous devez d\'abord payer la première tranche du Minerval';
                                        } elseif ($bloque_tranche_precedente) {
                                            $texte_blocage = 'Bloqué jusqu\'au payement de la tranche ' . ($numero_tranche - 1);
                                            $message_blocage = 'Vous devez d\'abord payer la tranche précédente';
                                        }
                                    }
                                ?>
                                    <div class="frais-item-paiement <?= $isSelected ? 'selected' : '' ?> <?= $tranche_bloquee ? 'frais-item-bloque' : '' ?>"
                                        data-bloquee="<?= $tranche_bloquee ? '1' : '0' ?>"
                                        onclick="<?= $tranche_bloquee ? 'afficherMessageTrancheRequise(\'' . addslashes($message_blocage) . '\')' : 'selectionnerFrais(' . $frais['id_frais'] . ', \'' . htmlspecialchars(addslashes($frais['type_frais'])) . '\', ' . $frais['montant'] . ', ' . ($frais['taux_change'] ?? $taux_defaut) . ')' ?>">
                                        <div class="frais-item-header">
                                            <h5><?= htmlspecialchars($frais['type_frais']) ?></h5><span class="frais-montant-item"><strong class="montant-usd-item">$<?= number_format($frais['montant'], 2) ?></strong><small class="montant-fc-item"><?= number_format($montant_fc, 0, ',', ' ') ?> FC</small></span>
                                        </div>
                                        <?php if ($tranche_bloquee): ?>
                                            <small class="taux-info-item text-warning fw-semibold"><?= htmlspecialchars($texte_blocage) ?></small>
                                        <?php else: ?>
                        ...
                        <?php if (!empty($frais_payes)): ?>
                            <div class="section-payes-header mt-4 mb-3">
                                <h4 class="text-success mb-0"><i class="fas fa-check-circle"></i> Déjà payés (<?= count($frais_payes) ?>)</h4>
                            </div>
                            <div class="frais-list payes">
                                <?php foreach ($frais_payes as $frais): ?>
                                    <div class="frais-item-paiement frais-deja-paye">
                                        <div class="frais-item-header">
                                            <div>
                                                <h5><?= htmlspecialchars($frais['type_frais']) ?></h5>
                                                <small class="frais-payes-date">Payé le <?= date('d/m/Y', strtotime($frais['date_paiement'])) ?></small>
                                            </div>
                                            <span class="badge-paye-sm"><i class="fas fa-check"></i> Payé</span>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                    ...
                    <div class="paiement-column">
                        ...
                        <div id="stepFormulaire">
                            <div class="paiement-card">
                                <div class="paiement-card-header">
                                    <h3><i class="fas fa-credit-card"></i> Effectuer le paiement</h3>
                                </div>
                                <div class="paiement-card-body">
                                    <div class="no-frais-selected" id="noFraisSelected"><i class="fas fa-hand-pointer"></i>
                                        <h4>Sélectionnez un frais</h4>
                                        <p>Cliquez sur un frais à gauche.</p>
                                    </div>
                                    <form id="formPaiement" style="display:none;" onsubmit="return initierPaiement(event)">
                                        ...
                                        <div class="frais-details-paiement">
                                            <h5 id="detailTypeFrais">—</h5>
                                            <div class="detail-montants">
                                                <div class="detail-montant-usd"><span class="detail-label">USD</span><strong id="detailMontantUSD">$0.00</strong></div>
                                                <div class="detail-montant-fc"><span class="detail-label">FC</span><strong id="detailMontantFC">0 FC</strong></div>
                                            </div><small class="detail-taux" id="detailTaux">Taux : 1$ = <?= $taux_defaut ?> FC</small>
                                        </div>
                                        ...
                                        <div class="recap-paiement">
                                            <h6><i class="fas fa-receipt"></i> Récapitulatif</h6>
                                            <div class="recap-ligne"><span>Frais</span><span id="recapFrais">—</span></div>
                                            <div class="recap-ligne"><span>Total</span><strong id="recapMontant">—</strong></div>
                                        </div>
                            ...
                            <div class="verification-card">
                                ...
                                <div class="verif-body">
                                    <div class="verif-info">
                                        <p><strong>Référence :</strong> <span id="verifRef">—</span></p>
                                        <p><strong>Montant :</strong> <span id="verifMontant">—</span></p>
                                        <p><strong>Statut :</strong> <span id="verifStatut" class="badge bg-warning">En attente</span></p>
                                    </div>
                                    <div class="verif-progress mt-3">
                                        <div class="progress">
                                            <div class="progress-bar progress-bar-striped progress-bar-animated" id="verifProgress" style="width:0%"></div>
                                        </div><small class="text-muted mt-1">Temps restant : <span id="verifTimer">60s</span></small>
                                    </div>
                        ...
                        <div id="stepSucces" style="display:none;">
                            <div class="succes-card text-center">
                                ...
                                <div class="succes-details">
                                    <div><span>Référence</span><strong id="succesRef">—</strong></div>
                                    <div><span>Montant</span><strong id="succesMontant">—</strong></div>
                                </div>
                        ...
                        <div id="stepEchec" style="display:none;">
                            <div class="echec-card text-center">
                                <div class="echec-icon"><i class="fas fa-times-circle"></i></div>
                                <h2>Paiement échoué</h2>
                                <p id="echecMessage">—</p><button class="btn btn-primary mt-3" onclick="reessayer()"><i class="fas fa-redo"></i> Réessayer</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function afficherMessageTrancheRequise(message) {
            Swal.fire({
                icon: 'warning',
                title: 'Paiement requis',
                text: message || 'Vous devez d\'abord payer la tranche précédente',
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (tel) tel.addEventListener('input', function() {
                ...
                let o = '—';
                if (t.length >= 9) {
                    const p = t.substring(t.length - 9, t.length - 7);
                    if (['84', '85'].includes(p)) o = 'Orange Money';
                    else if (['97', '98', '99'].includes(p)) o = 'Airtel Money';
                    else if (['82', '81', '83', '86'].includes(p)) o = 'Vodacom M-Pesa';
                }
                document.getElementById('operateurNom').textContent = o;
                document.getElementById('operateurDetecte').style.display = t.length > 0 ? 'flex' : 'none';
            });
            <?php if ($frais_selected > 0): ?>
                const el = document.querySelector(`.frais-item-paiement[onclick*="${<?= $frais_selected ?>}"]`);
                if (el) el.click();
            <?php endif; ?>
        });
        ...
        async function initierPaiement(event) {
            ...
            const m = fraisActuel.montant;
            const t = fraisActuel.taux;
            const montant = devise === 'USD' ? '$' + m.toFixed(2) : (m * t).toLocaleString('fr-FR') + ' FC';
            const conf = await Swal.fire({
                title: 'Confirmer le paiement ?',
                html: `<p>Frais : <strong>${fraisActuel.nom}</strong></p><p>Montant : <strong style="color:#7c3aed;font-size:1.2em;">${montant}</strong></p><p>Téléphone : <strong>${tel}</strong></p>`,
                icon: 'question',
            ...
            try {
                ...
                fd.append('id_frais', fraisActuel.id);
                fd.append('devise', devise);
                fd.append('telephone', tel);
                fd.append('montant_usd', m);
                fd.append('taux_change', t);
                const resp = await fetch('../api/istam/initier_paiement.php', {
                    method: 'POST',
                    body: fd
                });
                ...
                afficherEchec('Erreur de connexion.');
            }
            return false;
        }
            ...
            pollingInterval = setInterval(async () => {
                ...
                try {
                    ...
                    if (data.status === 'successful') {
                        clearIntervals();
                        afficherSucces(notreRef, data.montant);
                    } else if (data.status === 'failed' || data.status === 'cancelled') {
                        clearIntervals();
                        afficherEchec('Paiement refusé par l\'opérateur.');
                    }
                } catch (e) {}
                if (tentatives >= max) {
                    clearIntervals();
                    document.getElementById('verifTitle').textContent = 'Délai dépassé';
                    document.getElementById('verifStatut').textContent = 'En attente longue';
                    document.getElementById('verifActions').style.display = 'block';
                    document.getElementById('verifSpinner').style.display = 'none';
                }
            }, 3000);
        }
        ...
        function demarrerCountdown() {
            tempsRestant = 60;
            document.getElementById('verifTimer').textContent = '60s';
            countdownInterval = setInterval(() => {
                tempsRestant--;
                document.getElementById('verifTimer').textContent = tempsRestant + 's';
                if (tempsRestant <= 10) document.getElementById('verifActions').style.display = 'block';
                if (tempsRestant <= 0) clearInterval(countdownInterval);
            }, 1000);
        }
        ...
        async function verifierManuellement() {
            ...
            if (!gw || gw === '—') return;
            document.getElementById('verifSpinner').style.display = 'block';
            document.getElementById('verifCheck').style.display = 'none';
            ...
            try {
                ...
                if (data.status === 'successful') {
                    clearIntervals();
                    afficherSucces(data.notre_ref, data.montant);
                } else if (data.status === 'failed') {
                    afficherEchec('Paiement refusé.');
                } else {
                    document.getElementById('verifTitle').textContent = 'En attente';
                    document.getElementById('verifSpinner').style.display = 'none';
                    Swal.fire({
                        icon: 'info',
                        title: 'En attente',
        ...
        function afficherEchec(msg) {
            document.getElementById('stepVerification').style.display = 'none';
            document.getElementById('stepEchec').style.display = 'block';
            document.getElementById('echecMessage').textContent = msg;
        }

        function reessayer() {
            document.getElementById('stepEchec').style.display = 'none';
            document.getElementById('stepVerification').style.display = 'none';
            document.getElementById('stepFormulaire').style.display = 'block';
            clearIntervals();
        }
