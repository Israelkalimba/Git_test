...
if (!isset($_SESSION['logged_in']) || $_SESSION['user_role'] !== 'etudiant') { http_response_code(403); echo json_encode(['success'=>false,'message'=>'Non autorisé']); exit(); }
...
$etu = $stmt->fetch();
if (!$etu) { echo json_encode(['success'=>false,'message'=>'Étudiant introuvable']); exit(); }
...
if ($id_frais <= 0 || $montant_usd <= 0 || empty($telephone)) { echo json_encode(['success'=>false,'message'=>'Paramètres invalides']); exit(); }
...
if ($stmt->fetch()['total'] > 0) { echo json_encode(['success'=>false,'message'=>'Ce frais est déjà payé']); exit(); }
...
curl_setopt_array($ch, [CURLOPT_RETURNTRANSFER=>true, CURLOPT_HTTPHEADER=>['Authorization: Bearer '.$api_key,'Content-Type: application/json','Accept: application/json'], CURLOPT_POST=>true, CURLOPT_POSTFIELDS=>json_encode(['amount'=>(string)$montant_api,'currency'=>$devise,'phone'=>$telephone,'external_reference'=>$reference,'description'=>'Frais académique ISTAM','customer_name'=>$_SESSION['user_nom']??'Étudiant']), CURLOPT_TIMEOUT=>30]);
...
if ($httpCode === 201) {
    ...
    if (!empty($gateway_ref)) {
        ...
        $op = 'Mobile Money'; $tc = preg_replace('/[^0-9]/','',$telephone);
        if(strlen($tc)>=9){$p=substr($tc,-9,2);if(in_array($p,['81','82','83','84','85']))$op='Orange Money';elseif(in_array($p,['97','98','99']))$op='Airtel Money';elseif(in_array($p,['80','90','91']))$op='Vodacom M-Pesa';}
    ...
    } else { echo json_encode(['success'=>false,'message'=>'Réponse API invalide']); }
} else {
    $stmt = $db->prepare("UPDATE paiements SET statut = 'echec' WHERE id_paiement = :id");
    $stmt->execute(['id'=>$id_paiement]);
    $err = json_decode($response, true);
    echo json_encode(['success'=>false,'message'=>$err['error']??$err['message']??'Erreur API ('.$httpCode.')']);
}